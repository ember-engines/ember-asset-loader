...
placed into the asset manifest. This defaults to `bundles`. Each bundle is a directory containing files that will be be downloaded when the bundle is requested. You are responsible for getting the right files into those directories.
...
This addon doesn't perform manifest generator just by virtue of being installed because there is no convention for

...
AssetManifestGenerator.prototype.build = function () {
  ...
  // empty manifest.
  try {
    console.log('asset-manifest-generator: trying to get existing manifest....');
    existingManifest = fs.readJsonSync(path.join(existingManifestPath, 'asset-manifest.json'))
  } catch (err) {
    console.log('asset-manifest-generator: creating empty manifest because error was thrown');
    existingManifest = { bundles: {} };
  }

  console.log(`asset-manifest-generator: filesToIgnore: ${JSON.stringify(filesToIgnore)}`)
  ...
  var manifest = walk(inputPath).reduce(function (manifest, entry) {
    ...
    const shouldLog = assetName && assetName.includes('engine');

    shouldLog && console.log(`asset-manifest-generator: asset: ${bundleName}::${assetName}`);

    if (!bundleInfo.isValid) {
      shouldLog && console.log('asset-manifest-generator: returning because bundleInfo.isValid is false');
      return manifest;
    }

    if (bundleInfo.isNewBundle) {
      if (manifest.bundles[bundleName]) {
        shouldLog && console.log('asset-manifest-generator: throwing because bundle already exists in manifest');
        throw new Error('Attempting to add bundle "' + bundleName + '" to manifest but a bundle with that name already exists.');
      }

      shouldLog && console.log('asset-manifest-generator: adding new bundle to the manifest');
    ...
    // checking if it is a supported type.
      ...
      shouldLog && console.log(`asset-manifest-generator: asset is supported and not ignored: ${bundleName}::${assetName}`)
      // only add non-empty assets
      let fullPath = path.join(inputPath, entry);
      let contents = fs.readFileSync(fullPath, 'utf-8');

      if (contents.trim() !== '') {
        shouldLog && console.log(`asset-manifest-generator: adding asset: ${bundleName}::${assetName}`);
        manifest.bundles[bundleName].assets.push({
          uri: generateURI(path.posix.join(prepend, entry)),
          type: assetType
        });
      } else {
        shouldLog && console.log(`asset-manifest-generator: DID NOT ADD ASSET BECAUSE FILE WAS EMPTY: ${bundleName}::${assetName}`);
      }
    } else {
      shouldLog && console.log(`asset-manifest-generator: DID NOT ADD ASSET: ${bundleName}::${assetName}`)
    }
...
function stringOrRegexMatch(matchers, value) {
  var bundleInfo = bundleInfoFromAssetEntry(value);
  const shouldLog = bundleInfo && bundleInfo.assetName && bundleInfo.assetName.includes('engine');
  shouldLog && console.log(`asset-manifest-generator: in stringOrRegexMatch for ${bundleInfo.bundleName}::${bundleInfo.assetName}`);

  for (var i = 0; i < matchers.length; i++) {
    var matcher = matchers[i];

    if (typeof matcher === 'string' && matcher === value) {
      shouldLog && console.log(`asset-manifest-generator: returning true from stringOrRegexMatch: matcher: ${matcher}`);
      return true;
    } else if (matcher instanceof RegExp && matcher.test(value)) {
      shouldLog && console.log(`asset-manifest-generator: returning true from stringOrRegexMatch: matcher: ${JSON.stringify(matcher)}`);
      return true;
    }
  }

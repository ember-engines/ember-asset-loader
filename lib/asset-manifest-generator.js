...
AssetManifestGenerator.prototype.build = function () {
  ...
  // empty manifest.
  try {
    existingManifest = fs.readJsonSync(
      path.join(existingManifestPath, 'asset-manifest.json')
    );
  } catch (err) {
  ...
  var manifest = walk(inputPath).reduce(function (manifest, entry) {
    ...
    // checking if it is a supported type.
      ...
      supportedTypes.indexOf(assetType) !== -1 &&
      !stringOrRegexMatch(filesToIgnore, entry)
    ) {

...
AssetManifestGenerator.prototype.build = function () {
  ...
  var manifest = walk(inputPath).reduce(function (manifest, entry) {
    ...
    const shouldLog = assetName.includes('engine');
...
function stringOrRegexMatch(matchers, value) {
  ...
  const shouldLog = bundleInfo.assetName.includes('engine');
  ...
  for (var i = 0; i < matchers.length; i++) {
    ...
    if (typeof matcher === 'string' && matcher === value) {
      console.log(`asset-manifest-generator: returning true from stringOrRegexMatch: matcher: ${matcher}`);
      return true;
    } else if (matcher instanceof RegExp && matcher.test(value)) {
      console.log(`asset-manifest-generator: returning true from stringOrRegexMatch: matcher: ${JSON.stringify(matcher)}`);
      return true;
    }
  }

...
AssetManifestInserter.prototype = Object.create(
  BroccoliCachingWriter.prototype
);
...
AssetManifestInserter.prototype.build = function () {
  ...
  try {
    ...
    console.warn(
      '\n\nWarning: Unable to read asset-manifest.json from build with error: ' +
        error
    );
    console.warn(
      'Warning: Proceeding without generated manifest; you will need to manually provide a manifest to the Asset Loader Service to load bundles at runtime. If this was intentional you can turn this message off via the `noManifest` flag.\n\n'
    );
    manifest = { bundles: {} };
  }
  ...
  if (fs.existsSync(indexFilePath)) {
    ...
    if (index !== this.lastIndex) {
      ...
      fs.writeFileSync(
        path.join(this.outputPath, this.options.indexName),
        index
      );
    }
  }
  ...
  if (fs.existsSync(testIndexFilePath)) {
    var testIndexFile = fs.readFileSync(testIndexFilePath, {
      encoding: 'utf8',
    });
    ...
    if (testIndex !== this.lastTestIndex) {
      ...
      fs.writeFileSync(
        path.join(this.outputPath, 'tests', 'index.html'),
        testIndex
      );
    }
  }

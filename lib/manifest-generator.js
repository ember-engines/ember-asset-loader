...
var ManifestGenerator = Addon.extend({
  ...
  contentFor: function (type, config) {
    ...
    if (type === 'head-footer' && !(options && options.noManifest)) {
      ...
      return (
        '<meta name="' + metaName + '" content="%GENERATED_ASSET_MANIFEST%" />'
      );
    }
  },
  ...
  postprocessTree: function (type, tree) {
    ...
    var filesToIgnore = (
      (this.manifestOptions && this.manifestOptions.filesToIgnore) ||
      []
    ).concat(assetLoaderOptions.filesToIgnore || []);
    ...
    var treeWithInsertedManifest = insertAssetManifest(treeWithManifest, {
      indexName: indexName,
    });

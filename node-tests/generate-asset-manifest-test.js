...
describe('generate-asset-manifest', function () {
  ...
  let verifyAssetManifest = co.wrap(function* verifyAssetManifest(
    ...
    assert.equal(
      outputFiles.length,
      originalFiles.length + 2,
      'output files has two more files than originally'
    );

    assert.equal(
      originalFiles.indexOf('asset-manifest.json'),
      -1,
      'original files does not contain an asset manifest'
    );
    assert.notEqual(
      outputFiles.indexOf('asset-manifest.json'),
      -1,
      'output files does contain an asset manifest'
    );

    assert.equal(
      originalFiles.indexOf('assets/node-asset-manifest.js'),
      -1,
      'original files does not contain a Node asset manifest module'
    );
    assert.notEqual(
      outputFiles.indexOf('assets/node-asset-manifest.js'),
      -1,
      'output files does contain a Node asset manifest module'
    );
  ...
  it(
    'merges an existing manifest into the new one',
    co.wrap(function* () {
      ...
      assert.equal(
        ...
        'output files has one more file than originally'
      );

      assert.notEqual(
        originalFiles.indexOf('asset-manifest.json'),
        -1,
        'original files does contain an asset manifest'
      );
      assert.notEqual(
        outputFiles.indexOf('asset-manifest.json'),
        -1,
      ...
      var manifest = fs.readJsonSync(manifestFile);
      var expectedManifest = fs.readJsonSync(
        path.join(manifestsPath, 'full-plus-existing.json')
      );
      assert.deepEqual(manifest, expectedManifest);
    })
  );
